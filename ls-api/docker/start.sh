...
# run startup script from project
...
if [ -d "/root/post_boot_scripts/" ]; then
  # make scripts executable incase they aren't
  chmod -Rf 750 /root/post_boot_scripts/*; sync;
  # run scripts in number order
  for i in `ls /root/post_boot_scripts/`; do /root/post_boot_scripts/$i ; done
else
  echo "Can't find script directory"
fi
